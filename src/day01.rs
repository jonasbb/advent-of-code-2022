...
//! For that, their favorite snack is a special type of _star_ fruit that only grows deep in the jungle.
//! The Elves have brought you on their annual expedition to the grove where the fruit grows.
//!
//! To supply enough magical energy, the expedition needs to retrieve a minimum of _fifty stars_ by December 25th.
...
//! One important consideration is food - in particular, the number of _Calories_ each Elf is carrying (your puzzle input).
//!
...
//! In the example above, this is _`24000`_ (carried by the fourth Elf).
//!
...
//!
//! To avoid this unacceptable situation, the Elves would instead like to know the total Calories carried by the _top three_ Elves carrying the most Calories.
...
//! The sum of the Calories carried by these three elves is _`45000`_.
//!
...
fn part2(input: &[Vec<u32>]) -> u32 {
    let mut sums = input.iter().map(|v| v.iter().sum()).collect_vec();
    sums.sort_unstable();
    sums[sums.len() - 3..].iter().sum()
}

#[cfg(test)]
static TEST_INPUT_1: &str = r"1000
2000
3000
...
fn test_part1() -> Result<()> {
    ...
    assert_eq!(24000, part1(&values));
    Ok(())
}

#[test]
fn test_part1_solution() -> Result<()> {
    let values = input_generator(include_str!("../input/2022/day1.txt").trim());
    assert_eq!(74711, part1(&values));
    Ok(())
}
